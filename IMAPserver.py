...
class IMAPMailbox(object):
  ...
  def fetch(self, messages, uid):
    ...
    for seq, filename in messagesToFetch.items():
      uid = self.getUID(seq)
      flags = self.metadata['flags'].get(uid, [])
      yield seq, MaildirMessage(open(filename).read(), uid, flags)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--port", dest ="port", default="2525", help="Port number")
    ...
    passwordFile = os.path.join(storage, 'passwords.txt')
    ...
    print('IMAP Server running in:', port)
    reactor.run()